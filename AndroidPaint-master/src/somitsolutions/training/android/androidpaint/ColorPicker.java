    	...
    	if(v == okBtn) {
    		...
    		if( redValue>=0 && redValue<=100) {
    			...
    			redSeekBar.setProgress(redProgress);
    				...
    				// do nothing �ｽ it will close on its own
    				}
    				})
    				.show(); 
    				}
	    	...
	    	if( blueValue>=0 && blueValue<=100) {
	     	...
	     	Integer color = Color.rgb(redProgress, greenProgress, blueProgress);
	    	tableLayout.getRootView().setBackgroundColor(color);
	    	
	    	if(!((redValue > 100) || (greenValue>100) || (blueValue >100))){
	    	Intent i = new Intent();
	    	i.putExtra("somitsolutions.training.android.colorpicker.color_of_the_shape",color.toString());
	    	AndroidPaint.mColor = color;
